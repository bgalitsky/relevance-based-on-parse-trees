...
public class PhraseExtractorForSearcher {
	private static Matcher matcher;
	private static Logger LOG = Logger
			.getLogger("oracle.cloud.bots.document_searcher.PhraseExtractorForSearcher");
	...
	public PhraseExtractorForSearcher(Matcher matcherFromTK) {
	    matcher =matcherFromTK;
    }

	public PhraseExtractorForSearcher() {
	    matcher = new Matcher();
    }
	...
	public EntityExtractionResult extractEntities(String para){
		...
		for(List<ParseTreeNode> phrase: phrases){
			...
			if (!phrase.get(0).getPhraseType().equals("NP") && !phrase.get(0).getPhraseType().equals("VP") 
					&& !phrase.get(0).getPhraseType().equals("FRAG"))	
				bAccept = false;
